...
class Font extends PDFObject
{
     ...
     * @return string
     */
    public function translateChar($char, $use_default = true, $a)
    {
        $dec = hexdec(bin2hex($char));
        //$res = $char.'*----*'.bin2hex($char).'*----*'.$dec;
        if (array_key_exists($dec, $this->table)) {
            $char = $this->table[$dec];
        } else {
            $char = ($use_default ? self::MISSING : $char);
        }
        //echo $char.'----**'.$res.'----**'.PHP_EOL;
        //file_put_contents('1.txt',$char.'----**'.$res.'----**'.PHP_EOL,FILE_APPEND);
        return $char;
    }
        ...
        */
        ...
        foreach ($commands as $command) {
            switch ($command[PDFObject::TYPE]) {
                ...
                case '<':
                    //$command[PDFObject::COMMAND] == 190B0B764B263E7C
                    //解读转换：汉字这里解析
                    // Decode hexadecimal.
                    $text = self::decodeHexadecimal('<' . $command[PDFObject::COMMAND] . '>');//  \x19\v\vvK&>|
         ...
         */
         ...
         foreach ($words as &$word) {
              ...
              //if($words) dump($word.'====='.$word1);
         }

         //if($words) dump($a, implode(' ', $words));
         return implode(' ', $words);//返回的是汉字
    }
    ...
    protected function decodeContent($text, &$unicode)
    {
        if ($this->has('ToUnicode')) {
            ...
            if ($bytes) {
                ...
                for ($i = 0; $i < $length; $i += $bytes) {
                    ...
                    //解读：将二进制数据流转为汉字
                    if (($decoded = $this->translateChar($char, false, 11)) !== false) {
                        $char = $decoded;
                        ...
                        foreach ($fonts as $font) {
                            if ($font instanceof Font) {
                                if (($decoded = $font->translateChar($char, false, 22)) !== false) {
        ...
        /*if (!$unicode) {
            ...
            if ($this->get('Encoding') instanceof Element && $this->get('Encoding')->equals('MacRomanEncoding')) {
                $text = mb_convert_encoding($text, 'UTF-8', 'Mac');
            } else {
        ...
        if (strpos($text, '年化平均') !== false && $this->flag == 0) $this->flag = 1;
        //if ($unicode) dump($text .'----');//$this->flag
        return $text;
    }
}
