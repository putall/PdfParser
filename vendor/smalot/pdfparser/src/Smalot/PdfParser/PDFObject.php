...
class PDFObject
{
    ...
    {
        dd($this->content);
        //解读：$this->content是 TD、TJ形式的数据，这里返回汉字
        $text                = '';
        ...
        foreach ($sections as $section) {
            ...
            foreach ($commands as $command) {
                switch ($command[self::OPERATOR]) {
                    // set character spacing设置字符间距
                    case 'Tc':
                        break;
                    // move text current point将文本移动到当前点
                    case 'Td':
                        ...
                        if ((floatval($x) <= 0) || ($current_position_td['y'] !== false && floatval($y) < floatval($current_position_td['y']))) {
                            // vertical offset
                            $text .= "\n";
                        } elseif ($current_position_td['x'] !== false && floatval($x) > floatval($current_position_td['x'])) {
                            // horizontal offset
                            $text .= ' ';
                        }
                        $current_position_td = array('x' => $x, 'y' => $y);
                        break;
                    // move text current point and set leading移动文本当前点并设置前导
                    case 'TD':
                        $args = preg_split('/\s/s', $command[self::COMMAND]);
                        $y    = array_pop($args);
                        $x    = array_pop($args);
                    ...
                    // set super/subscripting text rise设置超级/订阅文本上升
                    case 'Ts':
                        break;

                    // set word spacing设置字距
                    case 'Tw':
                        break;

                    // set horizontal scaling设置水平缩放
                    case 'Tz':
                        $text .= "\n";
                        break;

                    // move to start of next line移到下一行的开头
                    case 'T*':
                        $text .= "\n";
                        //echo 'T*'.'----'.$text.PHP_EOL.PHP_EOL;
                        break;
                        ...
                        if (!is_null($page)) {
                            ...
                            $xobject = $page->getXObject($id);
                             ...
                             if ( is_object($xobject) && $xobject instanceof PDFObject && !in_array($xobject->getUniqueId(), self::$recursionStack) ) {
                                 // Not a circular reference.不是循环引用
